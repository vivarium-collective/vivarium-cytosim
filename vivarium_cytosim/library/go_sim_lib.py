...
def make_directory(path, n=0):
    ...
    while n < 10000:
        ...
        n += 1
    raise Error(
        "failed to create new run directory on " + os.getenv("HOSTNAME", "unknown")
    )
...
def run(exe, conf, name, args=[]):
    ...
    if "SLRUM_JOB_ID" in os.environ or "LSB_JOBID" in os.environ:
        print("running in temp dir")
        wdir = make_temp_directory()
    else:
        wdir = make_directory(name)
        # wdir = make_directory(cdir+'/'+name)
    # print(wdir+name)
    # if os.environ.has_key('SLURM_JOB_ID') or os.environ.has_key('LSB_JOBID'):
    #     wdir = make_temp_directory()
    # else:
    #     wdir = make_directory(name)
    # os.chmod(wdir, 504) # may need this one?? to change permissions. right now I can't
    os.chdir(wdir)
...
def start(exe, conf, name, args=[]):
    ...
    if exe != "none":
        ...
        pid = subprocess.Popen(
            ["nohup", exe] + args, stdout=outfile, stderr=errfile
        ).pid
